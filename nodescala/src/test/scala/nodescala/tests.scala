...
class NodeScalaSuite extends FunSuite {
  ...
  test("test")
  {
    val all = Future.all(List(Future.never[Int]))
    try {
  ...
  test("all") {
    ...
    try {
      val all = Future.all(List(Future.never[Int]))
      val all2 = Future.all(List(Future { 1 }, Future { 2 },Future.never[Int], Future {throw new Exception}))
      assert (false)
    } catch {
      case t: Exception => // ok!
    }
  ...
  test("Server should serve requests") {
    ...
    def test(req: Request) {
      ...
      print(webpage.loaded.future.isCompleted);
