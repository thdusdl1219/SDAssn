...
//    val (req,exc) = Await.result(listener.nextRequest(),1 second)
//    val p = Promise[Unit]()
    val canceltoken = Future.run() {
      ctk => {
        def a : Future[Unit] = Future{
          if(ctk.nonCancelled) {listener.nextRequest() onSuccess { case (req, exc) => {respond(exc, ctk, handler(req)); a}}}
        }
        a
      }
    }
    Subscription(canceltoken, listenerSubscription)
  }
