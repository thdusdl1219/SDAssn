...
package object nodescala {
  ...
  def allout[T](fs: List[Future[T]]): Future[List[T]] =  async {
      var _fs = fs
      val r = ListBuffer[T]()
      while (_fs != Nil) {
        r += await { _fs.head }
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
     ...
     */
    def all[T](fs: List[Future[T]]): Future[List[T]] = allout(fs)    
    ...
    def run()(f: CancellationToken => Future[Unit]): Subscription = {
      val p = CancellationTokenSource()
      f(p.cancellationToken)
      p
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
     ...
     */
    def now: T = try { Await.result(f, 0 nanos) } catch { case e : TimeoutException => throw new java.util.NoSuchElementException }
   ...
   //   f onComplete (p.tryComplete(_))
      f.flatMap(t => Future(cont(Future(t))))
    }
    ...
    def continue[S](cont: Try[T] => S): Future[S] = {
      val p = Promise[S]()
      f onComplete ( x => p.completeWith(Future(cont(x))))
      p.future
    }
