...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    (for(l <- b.legalNeighbors) yield((l._1, l._2::history))).toStream
  }
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    explored: Set[Block]): Stream[(Block, List[Move])] = neighbors.filter(n => !explored.exists(n._1 == _))
  ...
  def from(initial: Stream[(Block, List[Move])],
      ...
      if(initial.isEmpty) Stream()
        ...
        val newStream = (for (i <- initial) yield { newNeighborsOnly(neighborsWithHistory(i._1, i._2), explored) }).flatten
        val newexplored = explored ++ newStream.map(_._1)
        (initial ++ from(newStream, newexplored))
