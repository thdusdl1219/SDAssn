...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted = {
    if(left.isEmpty && right.isEmpty) elem
      ...
      (if (rmR.retweets > elem.retweets) rmR else elem)
    }
    else if (right.isEmpty && !left.isEmpty) {
      val lmR = left.mostRetweeted
      (if (lmR.retweets > elem.retweets) lmR else elem)
    }
    else {
      val rmR = right.mostRetweeted
      val lmR = left.mostRetweeted
      if (lmR.retweets > rmR.retweets && lmR.retweets > elem.retweets) lmR
      else if (rmR.retweets > elem.retweets) rmR
      else elem
    }
  }
...
/**
  def aux (st:String, l:List[String]):Boolean = l match {
    case x::xs => st.contains(x) || aux(st, xs)
    case _ => false
  }
  **/

  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(tw => google.exists(tw.text.contains))
  //lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(tw => aux(tw.text, google))
